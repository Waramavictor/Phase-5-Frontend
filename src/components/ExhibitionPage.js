...
const ExhibitionPage = () => {
  ...
  const formikCreateEvent = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        const response = await fetch('http://127.0.0.1:5000/exhibitions', {
          method: 'POST',
          headers: {
  ...
  const formikUpdateEvent = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const response = await fetch(`http://127.0.0.1:5000/exhibitions/${formikUpdateEvent.values.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('authToken')}`
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/exhibitions/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
        }
  ...
  const handleCreateSuccess = (newEvent) => {
    ...
    history.push('/my-exhibitions'); // Redirect to MyExhibitionPage after successful creation
  };
